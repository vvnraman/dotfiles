...
The candidate source can be either a regular file (``<candidate>``) or a template
(``<candidate>.tmpl``). This lets us make os-specific or user-specific source files templates when
we need dynamic content, while still linking to the same runtime target name.

No-op fallback for fault tolerance
----------------------------------

Each symlink selector checks whether the computed candidate exists in source state before emitting
it. If neither source form exists, the template falls back to a no-op target file.
